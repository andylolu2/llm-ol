\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \draw[densely dashed,bend right=40] (l11) to (doc4);

        % line to separate the ontology into train and test
        \draw[dashed] ($(root) + (0, 0.4)$) -- ($(root) + (0, -2.8)$);
        ...
        % PART D: Evaluation
        \coordinate (start) at ($(doc4)!0.5!(doc5) + (0, -0.4)$);
        \coordinate (inter) at ($(start) + (6.8, -0.5)$);
