\begin{figure}[t]
    ...
    \begin{tikzpicture}
        ...
        \node[anchor=west,inner sep=0,align=left] (caption) at ($(root.west) + (0.4, 2.3)$) {\small Dataset};
        ...
        \node[concept,color=c0] (testOutRoot) at ($(doc1)!0.5!(doc2) + (6.9, 0.0)$) {};
        ...
        \draw[->] ($(doc1)!0.5!(doc2) + (4.6, 0)$) -- node [midway,above,align=center] {\scriptsize Sum and prune} ($(doc1)!0.5!(doc2) + (6.6, 0)$);
        ...
        \draw[color={black!30},dashed] ($(root) + (-0.8, 0)$) -- ($(root) + (12.4, 0)$);
        % Caption Part B
        \node[color={black!60},inner sep=0,rotate=90] at ($(l2) + (-0.7, 0)$) {\small Training};
        % Caption Part C
        \node[color={black!60},inner sep=0,rotate=90,align=center] at ($(l1) + (-0.7,0)$) {\small Evaluation /\\Inference};
        % Arrow connection llm train to llm inference.
        \draw[->,color={black!30}] (llm) |- ($(llm)!0.5!(llm1)$) -| (llm1);
        % Legend
        \node[doc,fill={black!50}] (docLegend) at ($(root) + (10.6, -1.5)$) {};
