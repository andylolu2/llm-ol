\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node[doc,fill=c3] (input) at ($(root) + (3.8, 1)$) {};
        \node[llm] (llm) at ($(input) + (\trainSpace, 0)$) {LLM};
        \node[concept] (sgRoot) at ($(llm) + (\trainSpace + 0.1, 0.6)$) {};
        ...
        \node[concept] (testOutRoot) at ($(input1)!0.5!(input2) + (9.0, 0.7)$) {};
        ...
        \draw[->] ($(input1)!0.5!(input2) + (5.1, 0)$) -- node [midway,above,align=center] {Sum and prune} ($(input1)!0.5!(input2) + (8.0, 0)$);

        % PART D: Evaluation
        \coordinate (start) at ($(doc1)!0.5!(doc2) + (0, -0.5)$);
        \coordinate (inter) at ($(start) + (6.8, -1.4)$);
