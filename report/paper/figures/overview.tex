\begin{figure}[t]
    ...
    \begin{tikzpicture}
        ...
        \begin{scope}[local bounding box=target2,shift={($(loss.east) + (0.7, -0.8)$)},outer sep=3pt]
            \node[concept,color=c0] (sgRoot) {};
            \node[concept,color=c6] (sg2) at ($(sgRoot) + ({90-\angleA-20}:{\nodeDistShort})$) {};
        ...
        \draw[->,color=c3] (target1) -- (loss);
        \draw[->,color=c5] (target2) -- (loss);
        ...
        \node[concept,color=c0] (testOutRoot) at ($(doc1)!0.5!(doc2) + (7.6, 0.0)$) {};
        ...
        \node[fill={black!20},outer sep=2pt] (snp) at ($(doc1)!0.5!(doc2) + (5.8, 0)$) {\scriptsize Sum and prune};
        ...
        \draw[->] ($(snp.west) + (-0.2, 0)$) to (snp) to ($(snp.east) + (0.3, 0)$);
